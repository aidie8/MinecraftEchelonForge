package org.aidie8.EchelonJavaSDK.EchelonSDK;
...
import static org.aidie8.EchelonJavaSDK.EchelonSDK.Echelon.deviceID;
import static org.aidie8.EchelonJavaSDK.EchelonSDK.Echelon.getDomain;
...
public class EchelonWebSocketClient extends WebSocketClient {
    ...
    public void onClose(int code, String reason, boolean remote) {
        ...
        Echelon.logger.warn("Close Reason " + reason);
    }

    @Override
    public void onError(Exception ex) {
        Echelon.logger.error("Error Occurred");
        Echelon.logger.error(ex);
        ex.printStackTrace();
    ...
    public void init(onConnectSocket onConnectSocket)
    {
        ...
        {
            Echelon.logger.warn("Socket is already connecting or reconnecting, Ignoring Init call...");
            return;
        }
        connecting = true;

        Echelon.logger.info("Creating WebSocket Container");
        Echelon.logger.info("Created WebSocket Container Successfully");

        Echelon.logger.info("Connecting to server");
        try {
            this.connectBlocking();
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        Echelon.logger.info("Connected to server, sending init message");
    ...
    public static void sendMessage(String message)
    {
        ...
        {
            Echelon.logger.warn("Client is not connected");
        }
    }
